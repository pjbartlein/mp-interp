...
subroutine enforce_mean(no_negatives, nctrl, ntargs, nsubint, tol, xm_targ, xd, xmiss)
! adjustes daily values to match target mean
    ...
    if (debug_em) write (debug_unit,'("no_negatives: ", l1)') no_negatives
    ...
    do k = 1, nctrl
        ...
        if (debug_em) write (debug_unit, *) xm_targ(k), xdm(k), xm_targ(k) - xdm(k)
        
        ! if xm(k) = 0.0, set all days to 0.0
        if (xm_targ(k) .eq. 0.0d0) then 
            if (debug_em) write(debug_unit, '(a)') "       k      ib      ie   ndays xm_targ"
            if (debug_em) write(debug_unit, '(4i8, g12.4)')  k, ib, ie, ie - ib + 1, xm_targ(k)
            ...
            if (debug_em) write(debug_unit, '( 4i8, 5g12.4)')  & 
            ...
            if (dabs(diff(k)) .gt. tol) then
                ...
                do j = ib, ie
                    ...
                    if (debug_em) write (debug_unit, '(2i8, 2g12.4)') j, jj, xd(j), nonzero_sum(k)
                end do 
                ...
                do j = ib, ie
                    ...
                    if (xd(j) .ne. 0.0d0) then 
                        ...
                        if (debug_em) write (debug_unit, '(2i8, 3g12.4)') j, jj, xd(j), xd_adjust
                    else
                        if (debug_em) write (debug_unit, '(2i8, 3g12.4)') j, jj, xd(j)
                    end if   
                end do
                ...
                if (debug_em) write (debug_unit, '(4g12.4)') xd_adjust_sum(k), xd_next_sum(k), xd_next_mean(k), new_diff(k)
            else  
