...
integer(4), parameter  :: ny = 1, nm = 18           ! number of years and months in a year
integer(4), parameter  :: nctrl = ny * nm           ! total number of control points
integer(4), parameter  :: maxtarg = 216             ! total number of days
...
do ivar = 1, nvars
    ...
    open (3, file = trim(interppath)//trim(dataname)//"_mon_test.csv")
    open (4, file = trim(interppath)//trim(dataname)//"_day_test.csv")
